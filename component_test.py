...
class args():
    def __init__(self):
        ...
        self.gene_sets_gtf_bin = '/share/lustre/dgrewal/Miscellaneous/component_tests/annot_pygenes_titan/component_test/Homo_sapiens.GRCh37.73.gtf.gene_models.binary'
        self.gene_sets_gtf = None
        self.save_gtf_as_bin = False
...
class check_requirements(unittest.TestCase):
    ...
    def test_verify_pygenes(self):
        #need to be in the componentdir to load jointpysam
        cmd = component_reqs.requirements['python'] + ' -c "import pygenes"'
        proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        cmdout, cmderr = proc.communicate()
        
        #if no error printed on stdout/stderr then its fine else fail
        if cmdout != '' or cmderr != '':
            err = cmdout + ' ' +cmderr
